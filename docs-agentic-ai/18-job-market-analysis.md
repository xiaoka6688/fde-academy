...
>
> **2026-09 增补**：岗位库已持续扩充至 **195 个**（[实时岗位列表](/jobs/)每周更新），本页顶部新增[市场动态快照](#市场动态快照2026-09)；
> 5-6 月的 14 个 JD 深度拆解与学习路线图仍然有效，保留不变。

---

## 市场动态快照（2026-09）

> 数据来源：本站岗位库（195 个在库岗位，采集自 BOSS直聘/猎聘/牛客/各公司官方招聘站，[查看完整列表](/jobs/)）。
> 标签统计基于每岗 2-3 个核心标签，反映的是岗位结构分布，与下文 14 个 JD 的逐条深度统计口径不同，不可直接对比。

### 岗位类别分布（195 个）

| 类别 | 岗位数 | 占比 | 5-6 月以来变化 |
|---|---|---|---|
| 大模型推理/部署 | 48 | 25% | 存量最大，需求稳定，9月新增腾讯/京东/寒武纪等 |
| 大模型应用/Agent | 42 | 22% | 持续扩张，Agent 岗已逼近推理岗体量，9月新增联想/网易/滴滴等 |
| 大模型算法/架构 | 37 | 19% | 具身智能方向明显增多 |
| AI 平台/基础设施 | 27 | 14% | GPU Infra/CUDA 岗位薪资上限最高 |
| AI 前沿部署工程师 | 22 | 11% | AI 安全/智能体攻防成为新的独立方向 |
| AI 解决方案/架构 | 19 | 10% | 金融/政务场景为主 |

### 热门技能标签 TOP 12（按出现岗位数）

| 排名 | 技能 | 出现岗位数 | 说明 |
|---|---|---|---|
| 1 | 大模型（通用） | 27 | 基础门槛词 |
| 2 | **AI Agent** | 21 | 应用侧第一关键词，仍在上升 |
| 3 | 多模态 | 13 | 视觉/语音/全模态融合需求扩大 |
| 4 | 推理优化 | 12 | 推理侧核心词 |
| 5 | 校招 | 11 | 秋招季校招岗占比高（2026 届为主） |
| 6 | **具身智能** | 9 | ⬆️ 增长最快的新方向（上海AI Lab/滴滴/荣耀/港中深等） |
| 7 | CUDA | 9 | 硬件层刚需，拉开薪资差距 |
| 8 | 推理引擎 | 8 | vLLM/SGLang 二次开发 |
| 9 | RAG | 8 | Agent 应用标配 |
| 10 | **AI安全** | 6 | ⬆️ 新独立方向：智能体攻防、对齐、超级对齐（上海AI Lab/字节/小米/智谱） |
| 11 | vLLM | 5 | 推理岗标配（与"推理引擎/推理优化"标签合并看更高） |
| 12 | RLHF | 4 | 对齐/后训练技能要求外溢到应用岗 |

### 2026-09 市场薪资锚点（公开摘要）

| 岗位方向 | 代表公司 | 薪资 | 观察 |
|---|---|---|---|
| 大模型推理引擎研发 | 金山云 | 50-80K·16薪 | 推理方向薪资天花板继续抬高 |
| AI Agent 开发/架构师 | 字节跳动 | 40-70K·15薪 | Agent 架构师与算法专家同薪级 |
| GPU 推理（自动驾驶） | 百度 | 40-60K·16薪 | 推理技能向垂直场景溢价 |
| AI Infra 研发（GPU 推理） | 携程 | 30-60K·15薪 | 传统互联网大厂全面设岗 |
| 大模型推理引擎-GPU | 云天励飞 | 35-65K·15薪 | 国产芯片厂商加入抢人 |
| 大模型算法（资深） | 猎聘高薪岗位 | 45-75K·15薪 | 5-10 年资深人才稀缺 |
| AI 解决方案架构师 | 中国电子云 / VITURE | 40-60K | 央企背景与硬件公司均在设岗 |

### 核心变化（对比 5-6 月）

1. **具身智能成为最大增量**：上海 AI Lab 具身智能中心全球招聘、滴滴/荣耀/港中大（深圳）密集设岗，VLA、世界模型、运动控制是关键词——"边缘部署 + 实时推理"的 FDE 技能栈直接迁移
2. **AI 安全从加分项变独立岗位**：智能体攻防、大模型内生安全、超级对齐岗位出现在头部实验室与大厂安全部门，硕博优先
3. **Agent 岗位量价齐升**：岗位数逼近推理岗（37 vs 42），架构师级 40-70K；要求从单一 RAG 演进到 MCP/多智能体协同/记忆管理
4. **推理方向薪资上限突破**：金山云 50-80K·16薪 创本库新高，vLLM/SGLang 二次开发 + CUDA 能力是分水岭
5. **办公 Agent 巨头对垒**（字节"豆包工作" vs 腾讯 WorkBuddy，2026-08）预示企业级 Agent 部署岗位将在 Q4 放量
