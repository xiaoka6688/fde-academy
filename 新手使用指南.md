...
**自定义域名**（已绑定，2026-08-20 完成）：

线上正式地址为 **https://fde.pojuai.com**（腾讯云 DNSPod CNAME 接入，SSL 由 Cloudflare 自动签发）。配置方式备忘：

1. 腾讯云 → 云解析 DNS → pojuai.com → 添加记录：主机记录 `fde`、类型 `CNAME`、记录值 `fde-academy-ba6.pages.dev`
2. Cloudflare Pages → fde-academy → **自定义域** → 设置自定义域 → 输入 `fde.pojuai.com` → 选"我的 DNS 提供商 / 开始 CNAME 设置" → 检查 DNS 记录
3. 状态变"活动 + SSL 已启用"即成功；`fde-academy-ba6.pages.dev` 备用地址继续有效
