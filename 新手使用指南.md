...
写 Markdown ──→ npm start（本地实时预览）──→ git push ──→ Cloudflare Pages 自动构建
                                                          │
                                                          ▼
                                              线上站点 https://fde-academy.pages.dev
                                              （其他分支推送 → 自动生成独立预览链接）
```
...
Cloudflare Pages 拉取仓库（npm ci + npm run build）
    ↓
线上站点 https://fde-academy.pages.dev/   （约 1~3 分钟后生效）
```

- **生产部署**：推送 `master` 分支即发布到生产域名
- **预览部署**：推送其他分支（如 `git push origin feature/xxx`）会自动生成 `https://<分支名>.fde-academy.pages.dev` 独立预览链接，不影响线上——改完直接发链接给别人看效果
...
2. 按提示到腾讯云 DNS 解析控制台，添加 CNAME 记录指向 `fde-academy.pages.dev`
